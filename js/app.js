...
$(document).ready(function () {
    ...
    function getPSITwentyFour(query_timestamp) {
        ...
        let getPSI = $.ajax({
            ...
            .done(function (msg) {
                ...
                let readings = msg.items[0].readings.psi_twenty_four_hourly;

                for(var i = 0; i < location_arr.length;i++){
                    $("#psi-" + location_arr[i]).html(styleReading(readings[location_arr[i]]));
                }
            
                $("#psi-main-current").html(readings[$("#psi-main-current").data("psi-main-current")]);
                ...
                $(".main-bg").css("background-color", getHex(readings[current]));
            })
            .fail(function (xhr, text) {
                var errorMessage = xhr.status + ': ' + xhr.statusText
                console.log('Error - ' + errorMessage);
    ...
    $('#btn-twenty-four-psi').click(function (e) {
        e.preventDefault();
        current_mode = "psi";
        getPSITwentyFour(timeConvert());
